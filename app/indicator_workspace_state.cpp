...
namespace trading::app
{
    namespace
    {
            ...
            std::string& error)
        {
            ...
            if (!input) {
                error = "지표 상태 파일을 열지 못했습니다: " + path.string();
                return false;
            }
            std::ostringstream buffer;
            buffer << input.rdbuf();
            if (!input.good() && !input.eof()) {
                error = "지표 상태 파일을 읽지 못했습니다: " + path.string();
                return false;
            }
            text = buffer.str();
            if (text.size() >= 3U &&
            ...
            std::string& error)
        {
            ...
            if (!target.parent_path().empty()) {
                std::filesystem::create_directories(
                    ...
                    fileError);
                if (fileError) {
                    error = "지표 상태 폴더 생성 실패: " + fileError.message();
                    return false;
                }
            }
            ...
            if (fileError) {
                ...
                error = "지표 상태 파일 교체 실패: " + fileError.message();
                return false;
            }
            error.clear();
            return true;
            ...
            std::string& error)
        {
            ...
            if (!parsed.ok || !parsed.value.IsObject()) {
                error = parsed.ok
                    ? "지표 상태 JSON 루트가 객체가 아닙니다."
                    : "지표 상태 JSON 파싱 실패: " + parsed.error;
                return false;
            }

            const json_lite::Value* indicators =
                Find(parsed.value, "indicators");
            if (indicators == nullptr || !indicators->IsArray()) {
                error = "지표 상태 JSON에 indicators 배열이 없습니다.";
                return false;
            }

            IndicatorWorkspaceState candidate;
            std::set<std::string> ids;
            for (const json_lite::Value& item : indicators->AsArray()) {
                if (!item.IsObject()) {
                    error = "지표 상태 항목이 객체가 아닙니다.";
                    return false;
                }
                ...
                if (spec.id.empty() || spec.type.empty()) {
                    error = "지표 상태의 id 또는 type이 비어 있습니다.";
                    return false;
                }
                ...
                if (!CreateIndicatorDefinition(spec, definition, error)) {
                    error = "지표 상태 복원 실패 " + spec.id + ": " + error;
                    return false;
                }
                definition.visible = LevelVisible(item, definition.visible);
                ApplyOutputStateArray(Find(item, "outputs"), definition);
                ApplyReferenceStateArray(Find(item, "references"), definition);
        ...
        std::string& error)
    {
        ...
        for (const IndicatorInstanceDefinition& definition : definitions) {
            ...
            types.insert(definition.spec.type);
        }
        ...
        std::string& diagnostic)
    {
        ...
        struct Candidate final
        {
            ...
            { &savedPath, IndicatorWorkspaceSource::Saved, "저장값" },
            { &legacyPath, IndicatorWorkspaceSource::Legacy, "이전 저장값" },
            { &defaultPath, IndicatorWorkspaceSource::Default, "기본값" }
        };

        std::string failures;
        for (const Candidate& candidate : candidates) {
            if (candidate.path->empty() || !Exists(*candidate.path)) continue;
            IndicatorWorkspaceState loaded;
            std::string error;
            if (LoadOne(*candidate.path, loaded, error)) {
                state = std::move(loaded);
                source = candidate.source;
                if (!failures.empty()) diagnostic = failures;
                return true;
            }
            if (!failures.empty()) failures += " / ";
            failures += std::string(candidate.label) + "=" + error;
        }

        diagnostic = failures.empty()
            ? "지표 상태 JSON 파일을 찾지 못했습니다."
            : failures;
        return false;
    }
        ...
        IndicatorWorkspaceSource source) noexcept
    {
        ...
        case IndicatorWorkspaceSource::Saved:
            return "직전 저장값";
        case IndicatorWorkspaceSource::Legacy:
            return "이전 저장값";
